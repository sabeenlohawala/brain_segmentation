...
from utils import load_brains, brain_coord, mapping
...
def main():
    ...
    if os.path.exists(idx_path):
        ...
        with open(idx_path, "rb") as f:
            train_images, train_masks, validation_images, validation_masks, test_images, test_masks = pickle.load(f)
    else:
        # get file names
        ...
        train_images, test_images, train_masks, test_masks = train_test_split(image_files, mask_files, test_size=0.2, random_state=SEED)
        validation_images, test_images, validation_masks, test_masks = train_test_split(test_images, test_masks, test_size=0.5, random_state=SEED)
        data = [train_images, train_masks, validation_images, validation_masks, test_images, test_masks]
    ...
    for mode_idx, mode in enumerate(['train', 'validation', 'test']):
                ...
                for i in range(brain.shape[d]):
                    ...
                    for j in range(256):
                        if (brain_slice[:,255-j]!=0).any():
                            ...
                            break
                    if cut_bottom_temp-cut_top_temp+1>height or cut_right_temp-cut_left_temp+1>width:
                    ...
                    brain_slice = brain_slice[cut_top_temp:cut_bottom_temp+1, cut_left_temp:cut_right_temp+1]
                    mask_slice = mask_slice[cut_top_temp:cut_bottom_temp+1, cut_left_temp:cut_right_temp+1]
                    ...
                    if height_temp<height:
                        ...
                        if (diff % 2) == 0:
                            ...
                            # cut_top_temp -= diff//2
                            brain_slice = np.pad(brain_slice, ((diff//2,diff//2),(0, 0)))
                            mask_slice = np.pad(mask_slice, ((diff//2,diff//2),(0, 0)))
                        # odd difference
                        else:
                            # cut_bottom_temp += diff//2
                            # cut_top_temp -= diff//2 + 1
                            brain_slice = np.pad(brain_slice, ((diff//2,diff//2+1),(0, 0)))
                            mask_slice = np.pad(mask_slice, ((diff//2,diff//2+1),(0, 0)))
                    if width_temp<width:
                        ...
                        if (diff % 2) == 0:
                            ...
                            brain_slice = np.pad(brain_slice, ((0, 0),(diff//2,diff//2)))
                            mask_slice = np.pad(mask_slice, ((0, 0),(diff//2,diff//2)))
                        # odd difference
                        else:
                            # cut_right_temp += diff//2
                            # cut_left_temp -= diff//2 + 1
                            brain_slice = np.pad(brain_slice, ((0, 0),(diff//2,diff//2+1)))
                            mask_slice = np.pad(mask_slice, ((0, 0),(diff//2,diff//2+1)))
                    ...
                    if mode == "train":
                        ...
                        image_mean, image_std = np.mean(brain_slice), np.std(brain_slice)
        ...
        if mode == "train":
            dataset_mean /= idx
            dataset_std /= idx
            np.save(f"{save_path_basic}/normalization_constants.npy", np.array([dataset_mean, dataset_std]))
