...
class Dice(nn.Module):
    ...
    def __init__(self, nr_of_classes: int, fabric: L.Fabric, smooth: float = 1e-7) -> None:
        ...
        pixel_counts = np.load(f'/om2/user/{DATA_USER}/nobrainer_data_norm/data_prepared_segmentation_{DATA_SIZE}/pixel_counts.npy')
        # NR_OF_CLASSES = 6
    ...
    def forward(self, y_true: torch.tensor, y_pred: torch.tensor):
        ...
        self.classDice = torch.mean(2. * class_intersect / (class_denom + self.smooth),axis=0)
        ...
        return dice_loss, self.classDice
...
class Classification_Metrics():
    ...
    def log(self, commit: bool = False):
        logging_dict = {
            ...
            # f"{self.prefix}/DICE/overall/new": sum(self.dice)/len(self.loss),
    ...
    def sync(self, fabric):
        ...
        assert self.Assert == torch.cuda.device_count(), f"Metrics Syncronization Did not Work. Assert: {self.Assert}, Devices {torch.cuda.device_count()}"