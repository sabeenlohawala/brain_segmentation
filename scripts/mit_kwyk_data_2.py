...
parser.add_argument(
    "--group_size",
    help="Number of volumes to split in each slice subset",
    type=int,
    required=False,
    default=1000,
)
parser.add_argument(
    "--group_num",
    help="Which slice subset, must be > 0",
    type=int,
    required=False,
)
...
def get_feature_label_pairs(features_dir=SOURCE_DIR_00, labels_dir=SOURCE_DIR_00, group_size=1000, group_num=None):
    """
    ...
    if group_num is not None:
        features = features[(group_num - 1)*group_size:group_num*group_size]
        labels = labels[(group_num - 1)*group_size:group_num*group_size]
...
def get_train_val_test_split():
    ...
    if GROUP_SIZE is not None and GROUP_NUM is not None:
        feature_label_subset_pairs = get_feature_label_pairs(features_dir=FEATURE_TRANFORM_DIR, labels_dir=LABEL_TRANFORM_DIR,group_size=GROUP_SIZE,group_num=GROUP_NUM)
        features_subset, labels_subset = zip(*feature_label_subset_pairs)
        features_subset = set(features_subset)
        labels_subset = set(labels_subset)
        train_features = list(set(train_features) & features_subset)
        train_labels = list(set(train_labels) & labels_subset)
        validation_features = list(set(validation_features) & features_subset)
        validation_labels = list(set(validation_labels) & labels_subset)
        test_features = list(set(test_features) & features_subset)
        test_labels = list(set(test_labels) & labels_subset)
...
def extract_kwyk_slices(max_shape):
    ...
    if GROUP_NUM is not None:
        percent_backgrounds_file = f"percent_backgrounds_{GROUP_SIZE}_{GROUP_NUM}.json"
    else:
        percent_backgrounds_file = "percent_backgrounds.json"
    with open(os.path.join(SLICE_DEST_DIR, percent_backgrounds_file), "w") as f:
