    ...
    for axis in range(3):
        for slice_idx in range(vol_shape[axis+1]):
            ...
            if np.sum(img) < 52428:
                ...
                if len(nonzero_indices[0]) == 0 or len(nonzero_indices[1]) == 0 or (np.max(nonzero_indices[0]) - np.min(nonzero_indices[0]) + 1 < 50) or (np.max(nonzero_indices[1]) - np.min(nonzero_indices[1]) + 1 < 50):
                    slices_nonbrain[axis, slice_idx] = 0
...
def get_shard_nonzero(shard_idx):
    ...
    get_vol_filter = get_vol_matthias if FIND_MATTHIAS_FILTER == 1 else get_vol_nonzero
    ...
    with Pool(processes=n_procs) as pool:
        slices_nonbrain = pool.starmap(
            ...
            [
                ...
                for shard_vol_idx in range(10) #range(vol_shape[0])
            ],
        )
