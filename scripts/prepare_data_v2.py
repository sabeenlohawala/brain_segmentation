...
parser.add_argument(
    "--nese",
    help="Wheter to write dataset to nese", 
    type=int, 
    required=False, 
    default=0
)
...
parser.add_argument(
    "--random_orientation", 
    help="Wheter to apply random rotation to all volumes before slicing",
    type=int,
    required=False,
    default=0
)
...
def main():
    ...
    if NESE:
        save_path_basic = f"/nese/mit/group/sig/data/{DATASET_NAME}"

    for mode in ['train','validation','test']:
        save_path_mode = f'{save_path_basic}/{mode}'
        os.makedirs(save_path_mode, exist_ok=True)
