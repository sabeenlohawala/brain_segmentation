...
for i, file in enumerate(img_files):
    ...
    p = create_plot(pred, '', color_range, fig_path=str(os.path.join(pred_dir,f"{file.split('.')[0]}_pred.png")))
    real_img = img_batch[i,:,:,:].squeeze()
    if len(real_img.shape) > 2:
        real_img = real_img.permute(1,2,0)
    cv2.imwrite(str(os.path.join(pred_dir,f"{file.split('.')[0]}_gray.png")),(real_img.numpy() * 255).astype(np.uint8))
