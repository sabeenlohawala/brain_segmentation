...
#     @torch.no_grad()
#     def logging(self, model, e: int, commit: bool, writer = None):
#         model.eval()
...
#                 img = np.array(img)
#                 if len(img.shape) == 3:
#                     writer.add_image(key, np.array(img), e, dataformats='HWC')
#                 elif len(img.shape) == 2:
#                     writer.add_image(key, np.array(img), e, dataformats='HW')

#     @torch.no_grad()
#     def logging_v1(self, model, e: int, commit: bool):
#         model.eval()
#         probs = model(self.brain_slices)
#         probs = probs.argmax(1)
#         probs = probs.cpu()
