...
from keras.layers import InputLayer, Conv2D, Conv2DTranspose, Identity, BatchNormalization, ReLU, MaxPool2D, Softmax
...
class Block(keras.layers.Layer):
    def __init__(self, out_ch, up=False,name='block'):
        ...
        self.conv2 = Conv2D(filters=out_ch,kernel_size=3,padding='same',data_format='channels_first')
...
class OriginalUnetTF(tf.keras.Model):
    ...
    def make(self, image_size=(160,192), batch_size=128):
        ...
        '''
        x = tf.keras.layers.Input(shape=(self.image_channels,*image_size),batch_size=batch_size)
        model = tf.keras.Model(inputs=[x], outputs=self.call(x), name='NobrainerUnetTF Summary')
        print(model.summary())
        return model
