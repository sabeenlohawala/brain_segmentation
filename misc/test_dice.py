...
def compare_params(save_path):
    single = torch.load(os.path.join(save_path, f"params_0_1.pt")).cpu()
    multi_0 = torch.load(os.path.join(save_path, f"params_0_2.pt")).cpu()
    multi_1 = torch.load(os.path.join(save_path, f"params_1_2.pt")).cpu()

    print(f"single vs multi_0: {torch.sum(single != multi_0)}")
    print(f"single vs multi_1: {torch.sum(single != multi_1)}")
    print(f"multi_0 vs multi_1: {torch.sum(multi_0 != multi_1)}")
...
def main():
    ...
    save_path = "/om2/user/sabeen/nobrainer_data_norm/test_dice_data/51class_2/"
    # compare_params(save_path)

    config.binary = False
    if config.binary:
        config.nr_of_classes = 2
        save_path = "/om2/user/sabeen/nobrainer_data_norm/test_dice_data/binary_loss/"
    ...
    image_0, mask_0, probs_0 = torch.load(
        os.path.join(save_path, "image_mask_probs_0.pt")
    )
    image_1, mask_1, probs_1 = torch.load(
        os.path.join(save_path, "image_mask_probs_1.pt")
    )

    # get class_intersect/class_union
    class_intersect_0, class_union_0 = torch.load(
        os.path.join(save_path, f"itersect_denom_0.pt")
    )
    class_intersect_1, class_union_1 = torch.load(
        os.path.join(save_path, f"itersect_denom_1.pt")
    )

    # get gathered
    class_intersect_gather_0, class_union_gather_0 = torch.load(
        os.path.join(save_path, f"itersect_denom_gather_0.pt")
    )
    class_intersect_gather_1, class_union_gather_1 = torch.load(
        os.path.join(save_path, f"itersect_denom_gather_1.pt")
    )

    # find class_intersect and class_union for batch_size = 2
    ...
    multi_intersect_sum = torch.sum(
        torch.concat((class_intersect_0, class_intersect_1)), axis=0
    )
    multi_denom_sum = torch.sum(torch.concat((class_union_0, class_union_1)), axis=0)
    ...
    print(
        f"gather_intersect_sum_0 == gather_intersect_sum_1: {torch.sum(gather_intersect_sum_0 == gather_intersect_sum_1)} / {config.nr_of_classes} are equal"
    )
    print(
        f"gather_denom_sum_0 == gather_denom_sum_1: {torch.sum(gather_denom_sum_0 == gather_denom_sum_1)} / {config.nr_of_classes} are equal"
    )
    # sanity_check: multi should equal gather_0 and gather_1
    print(
        f"multi_intersect_sum == gather_intersect_sum_0: {torch.sum(multi_intersect_sum == gather_intersect_sum_0)} / {config.nr_of_classes} are equal"
    )
    print(
        f"multi_denom_sum == gather_denom_sum_0: {torch.sum(multi_denom_sum == gather_denom_sum_0)} / {config.nr_of_classes} are equal"
    )
    # Q: does single = gather?
    print(
        f"single_intersect_sum == gather_intersect_sum_0: {torch.sum(gather_intersect_sum_0 == single_intersect_sum)} / {config.nr_of_classes} are equal"
    )
    print(
        f"single_denom_sum == gather_denom_sum_0: {torch.sum(gather_denom_sum_0 == single_denom_sum)} / {config.nr_of_classes} are equal"
    )
    # Q: does single = multi?
    print(
        f"single_intersect_sum == multi_intersect_sum: {torch.sum(multi_intersect_sum == single_intersect_sum)} / {config.nr_of_classes} are equal"
    )
    print(
        f"single_denom_sum == multi_denom_sum: {torch.sum(multi_denom_sum == single_denom_sum)} / {config.nr_of_classes} are equal"
    )
...
def generate_files(config, save_path):
    ...
    # a = [(name, param.data) for name, param in model.named_parameters() if param.requires_grad]
    # params = a[0][-1].data
    # torch.save(params, os.path.join(save_path,f'params_{fabric.global_rank}_{num_devices}.pt'))
    ...
    for epoch in range(1, 2):
        ...
        for i, (image, mask) in enumerate(train_loader):
            ...
            # loss, classDice = loss_fn(mask.long(),probs,debug=(fabric.global_rank == 0))
            loss, classDice = loss_fn(
                mask.long(), probs, debug=(fabric.global_rank == 0)
            )
            # print(f'Rank {fabric.global_rank} pre-backward loss: {loss.item()}')
            ...
            # print(f'Rank {fabric.global_rank} post-backward loss: {loss.item()}')
            losses.append(loss.item())
            optimizer.step()
            train_metrics.compute(
                mask.long(), probs, loss.item(), classDice, class_intersect, class_union
            )
            ...
            with torch.no_grad():
                ...
                all_loss = fabric.all_gather(loss)
                all_losses = fabric.all_gather(losses)

            print("saving files...")
            # save (brain,mask,pred) from each rank
            torch.save(
                [image.cpu(), mask.cpu(), probs.cpu()],
                os.path.join(save_path, f"image_mask_probs_{fabric.global_rank}.pt"),
            )
            # save (class_intersect, class_union) from each rank
            torch.save(
                [class_intersect.cpu(), class_union.cpu()],
                os.path.join(save_path, f"itersect_denom_{fabric.global_rank}.pt"),
            )
            # save (class_intersect_gather, class_union_gather)
            torch.save(
                [class_intersect_gather.cpu(), class_union_gather.cpu()],
                os.path.join(
                    save_path, f"itersect_denom_gather_{fabric.global_rank}.pt"
                ),
            )
